...
{
	mE_PrepLbMesh_rend.resize(6, mPoly->numHalfEdges() * 2);
	...
	for (int ei = 0; ei < mPoly->numHalfEdges(); ei++)
	{
		...
		mE_PrepLbMesh_rend(1, ei * 2 + 0) = he->src()->position().y;
		mE_PrepLbMesh_rend(2, ei * 2 + 0) = he->src()->position().z;
		mE_PrepLbMesh_rend(3, ei * 2 + 0) = 1e-3;
		mE_PrepLbMesh_rend(4, ei * 2 + 0) = 1e-3;
		mE_PrepLbMesh_rend(5, ei * 2 + 0) = 1e-3;

		mE_PrepLbMesh_rend(0, ei * 2 + 1) = he->dst()->position().x;
		mE_PrepLbMesh_rend(1, ei * 2 + 1) = he->dst()->position().y;
		mE_PrepLbMesh_rend(2, ei * 2 + 1) = he->dst()->position().z;
		mE_PrepLbMesh_rend(3, ei * 2 + 1) = 1e-3;
		mE_PrepLbMesh_rend(4, ei * 2 + 1) = 1e-3;
		mE_PrepLbMesh_rend(5, ei * 2 + 1) = 1e-3;
	}
}
